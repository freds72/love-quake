local ModelReader = function(pak)
    ...
    ::skip::
        ...
        unpack_array(function(node)
            ...
            local n={
                plane=5*node.planenum + plane_offset,
                mins={mins[0],mins[1],mins[2]},
                maxs={maxs[0],maxs[1],maxs[2]}
            }
            local flags = 0
    ...
    local function load_bsp(name)
        ...
        return unpack_map(bsp), entityReader:unpack(entities)
    end
    ...
    local function load_aliasmodel(name)
        local mem = pak:open(name)
    ...
    -- load model name from PAK
    local modelfs={}
    function modelfs:load(name)
        ...
        if not model then               
            ...
            elseif extension==".mdl" then
                ...
                local alias = load_aliasmodel(name)
                model = {
                    alias = alias            
                }
            else
